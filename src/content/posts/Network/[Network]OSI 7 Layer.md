...
그리고, 그에 맞춰 "백본 스위치, L7 Load Balancer 등등" 여러 네트워크 장비들이 쭈우욱~ 나열되어있습니다.  

이를 위해서는 `OSI7 Layer`의 구조 이해가 필요합니다. 이번 포스팅에서는 Layer의 이해와 등장하는 용어를 쉽게 설명하고
네트워크 아키텍쳐 구조에서 네트워크 장비들이 어떻게 사용되는지 찾아보고 알아보도록 합시다.
...
네트워크가 한개의 라인으로 되어있는데 N:M 구조라면? 고장났을때 모든 회선을 찾을때까지 다 교체를 해줘야합니다. 그래서 `OSI 7 Layer`가 필요한겁니다.  

또한, 네트워크 환경은 가장 정보보안에서 가장 취약한 부분이라고 할 수 있다.
취약점의 대부분 발견되는 위치는 망과 망사이 접점. 다중의 통신 채널에서 발생하게 됩니다.

돌아가서, `OSI 계층은 일종의 네트워크의 표준 모델`이라고 합니다.
...
- 특징
  ...
  4. Unique `MAC 주소` 사용 ➡️ 네트워크 카드로 부터 `고유하게 생성`
...
- 특징
  ...
  3. 오류제어: TTL(Time To Live) 및 헤더 체크섬 활용 
    > 💡 TTL (Time To Live)
    > 
    > 네트워크 패킷이 얼마나 오랫동안 네트워크 상에서 전송될 수 있는지르 제한하는 값
  > 
    > 동작원리
    > 1. 패킷 생성시 TTL 값 설정(예: 64, 128, 255) 
    > 2. 라우터를 통과시 TTL 값을  -1하여, 패킷이 얼마나 많은 라우터를 통과했는지 확인 
    > 3. TTL 값이 0이되면 패킷은 폐기가 된다. ICMP 메시지를 보낸다.
    > 
    > 목적
    > - 루프 방지: 라우팅이 잘못된 경로에서는 무한 순환을 방지
    > - 네트워크 혼잡방지: 오래된 패킷이 순환하는 것을 막아 네트워크 트래픽을 줄인다.
  4. 트래픽 제어 및 품질 관리
...
- 장비: L4 로드벨런서, 프록시 서버
- 프로토콜: TCP, UDP
- 목적: 데이터를 송신자와 수신자 간에 신뢰성 있게 전달하는 역할(어플리케이션 식별을 위한 포트번호 추가)
- 특징
  1. 신뢰성 있는 데이터 전송
  2. 흐름 제어
  3. 다중화 및 역다중화
